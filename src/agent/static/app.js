...
// D&D Characters Widget Rendering
async function loadDndCharacters() {
    const container = document.getElementById('dnd-characters-container');
    if (!container) return;
    ...
    try {
        ...
        characters.forEach((char) => {
            ...
            name.textContent = char.name || `Hero #${characters.indexOf(char) + 1}`;
...
async function init() {
    ...
    // Load initial D&D character rolls
    loadDndCharacters();
