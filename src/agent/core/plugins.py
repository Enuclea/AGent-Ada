...
def verify_plugin_ast_safety(plugin_path: Path) -> None:
    ...
    unless the plugin resides within the trusted, non-writable built-in agent plugins package path.
    """

    try:
        import agent.plugins
        import stat
        if hasattr(agent.plugins, "__path__"):
            for path_str in agent.plugins.__path__:
                resolved_path = Path(path_str).resolve()
                if Path(plugin_path).resolve().is_relative_to(resolved_path):
                    try:
                        # 1. Check package root permissions
                        st_root = resolved_path.stat()
                        if st_root.st_mode & stat.S_IWOTH:
                            continue
                        
                        # 2. Check target plugin directory and all subfiles recursively
                        target_dir = Path(plugin_path).resolve()
                        st_target = target_dir.stat()
                        if st_target.st_mode & stat.S_IWOTH:
                            continue
